...
import {
  primaryStudentSidebarItems,
  secondaryStudentSidebarItems,
} from "@/components/student/common/items";
import { footerLinks } from "@/data/contact";

function StudentDesktopSidebar({ pathname }: { pathname: string }) {
  ...
  return (
    <nav className="hidden lg:flex w-full h-full overflow-y-auto bg-[#fdfdfd] py-2 lg:py-4 px-2 lg:px-4 flex-col border-r border-[#E5E9EB]">
      ...
      <div className="flex flex-col gap-1.5">
        {primaryStudentSidebarItems.map((item) => (
          <Link
            key={item.id}
            href={item.href}
      ...
      <div className="flex flex-col space-y-2 mt-7">
        {secondaryStudentSidebarItems.map((item) => (
          <Link
            key={item.id}
            href={item.href}
...
function StudentMobileSidebar({ pathname }: { pathname: string }) {
    ...
    <aside className="lg:hidden">
      <div className="w-full p-2 bg-white border-b border-gray-100 flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <p className="text-lg font-medium text-gray-900">Student Portal</p>
        </div>
      </div>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex">
          ...
          />
          <div className="relative w-72 bg-white h-full overflow-y-auto p-4 flex flex-col gap-8 shadow-xl">
...
function StudentNavbar({ pathname }: { pathname: string }) {
  const [showUser, setShowUser] = useState(false);
  const userRef = useRef<HTMLDivElement>(null);
  const { userData } = useAuth();

  const allItems = [...primaryStudentSidebarItems, ...secondaryStudentSidebarItems];
  const title =
    ...
    <header className="bg-white border-b border-[#E5E9EB] p-3 lg:px-8 sticky to-0 z-10">
      ...
      {showUser && (
        ...
        >
          ...
          <Link href="/s/dashboard">Dashboard</Link>
          <Link href="/s/my-progress">My Progress</Link>
          <hr className="text-[#E1E1E1]" />
...
function StudentLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="w-full grid grid-cols-1 lg:grid-cols-5 xl:grid-cols-6 h-dvh overflow-hidden">
      <StudentDesktopSidebar pathname={pathname} />
      <StudentMobileSidebar pathname={pathname} />
      <div className="lg:col-span-4 xl:col-span-5 w-full h-dvh overflow-hidden flex flex-col">
        <StudentNavbar pathname={pathname} />
