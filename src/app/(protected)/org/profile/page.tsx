...
export default function OrgProfilePage() {
  ...
  const saveProfile = async (e: React.FormEvent) => {
    ...
    try {
      await fetchApi("/auth/update-my-profile", { method: "PUT", body: profileForm });
  ...
  const saveCollege = async (e: React.FormEvent) => {
    ...
    try {
      await fetchApi(`/colleges/${collegeId}`, { method: "PUT", body: collegeForm });
  ...
  const inputCls = "py-2.5 px-4 rounded-md border border-gray-200 w-full text-sm focus:outline-none focus:ring-2 focus:ring-primary/30";
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <Section title="Personal Information">
        <div className="text-sm text-gray-500 -mt-2">
          ...
          <span className="ml-2 text-xs bg-gray-100 px-2 py-0.5 rounded">Email cannot be changed</span>
        </div>
        <form onSubmit={saveProfile}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select name="gender" value={profileForm.gender} onChange={handleProfileChange} className={inputCls}>
            ...
            <div className="sm:col-span-2">
              ...
              <input name="address" value={profileForm.address} onChange={handleProfileChange}
                className={inputCls} placeholder="Your address" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              ...
              <div className="relative">
                <input name="currentPassword" type={showPasswords ? "text" : "password"}
            ...
            <div>
              ...
              <input name="newPassword" type={showPasswords ? "text" : "password"}
            ...
            <div>
              ...
              <input name="confirmPassword" type={showPasswords ? "text" : "password"}
            ...
            <div className="sm:col-span-2">
              <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
                <input type="checkbox" checked={showPasswords}
                  onChange={(e) => setShowPasswords(e.target.checked)} className="w-4 h-4" />
                Show passwords
              </label>
            </div>
          </div>
          ...
          <form onSubmit={saveCollege}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input name="name" value={collegeForm.name} onChange={handleCollegeChange}
                  className={inputCls} placeholder="College name" />
              </div>
              <div>
                <label className={labelCls}>Type</label>
                <select name="type" value={collegeForm.type} onChange={handleCollegeChange} className={inputCls}>
              ...
              <div>
                ...
                <input name="country" value={collegeForm.country} onChange={handleCollegeChange}
                  className={inputCls} placeholder="Country" />
              </div>
              <div>
                <label className={labelCls}>City <span className="text-red-500">*</span></label>
                <input name="city" value={collegeForm.city} onChange={handleCollegeChange}
                  className={inputCls} placeholder="City" />
              </div>
              <div className="sm:col-span-2">
                <label className={labelCls}>Street Address</label>
                <input name="streetAddress" value={collegeForm.streetAddress} onChange={handleCollegeChange}
                  className={inputCls} placeholder="Street address" />
              </div>
              <div className="sm:col-span-2">
                <label className={labelCls}>Description</label>
                <textarea name="description" value={collegeForm.description} onChange={handleCollegeChange}
                  className={`${inputCls} resize-none`} rows={3} placeholder="About your college" />
              </div>
            </div>
            <div className="mt-5 flex justify-end">
              <button type="submit" disabled={collegeSaving}
                className="bg-primary text-white text-sm py-2.5 px-6 rounded-lg hover:bg-primary-dark transition disabled:opacity-60">
